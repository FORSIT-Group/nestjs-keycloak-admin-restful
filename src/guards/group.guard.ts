...
export class GroupGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean | Promise<boolean> {
    const meta = this.reflector.get<{
      ...
      paramName: string
    }>(META_GROUPNAME, context.getHandler())
    try {
      ...
      if (request.scopes.includes(limitedScopeName)) {
        ...
        const groups: string[] = request.user.groups.filter((group: string) => {
          return group.startsWith(meta.groupName)
        })
        ...
        const groupIds: string[] = groups.map((group: string) => {
          return group.replace(meta.groupName, '')
        })
